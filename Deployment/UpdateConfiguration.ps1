...
$confileFilePath = "$rootPath\redis.windows.conf"
...
if($logFile.StartsWith(".")) {
	$logFile = (Join-Path $rootPath $logFile.SubString(1, $logFile.Length - 1)).ToString()
}
...
persistence-available $($e.PersistanceAvailable)
...
# The default is:
#
maxmemory-policy $($e.MaxMemoryPolicy)
...
no-appendfsync-on-rewrite $($e.NoAppendFsyncOnRewrite)
...
auto-aof-rewrite-percentage $($e.AutoAofRewritePercentage)
auto-aof-rewrite-min-size $($e.AutoAofRewriteMinSize)
...
# big latency spikes.
aof-rewrite-incremental-fsync $($e.AofRewriteIncrementalFsync)
